...
import { api, refreshSession, setAccessToken } from '../src/lib/api'
...
describe('api client token refresh', () => {
  ...
  it('shares one refresh call across concurrent 401s (single-flight)', async () => {
    // The backend rotates refresh tokens and treats reuse of a rotated
    // token as theft, revoking the whole family — so concurrent 401
    // retries must never trigger two refresh requests.
    let refreshCalls = 0
    let refreshed = false
  ...
  it('refreshSession resolves false without a session', async () => {
    ...
    // Concurrent boot-time callers (StrictMode double-mount) share one call.
...
describe('api client error shaping', () => {
  ...
  it('surfaces the detail string from FastAPI errors', async () => {
    vi.stubGlobal(
      'fetch',
