...
| `display-lg`, `subtitle`, `body-md` (marketing body) | ABC Favorit | **Instrument Sans** for `display-lg`/`subtitle` roles, **Inter** for `body-md`/UI roles — keep the "strict lane" rule from the original doc: never mix these two outside their assigned roles |
