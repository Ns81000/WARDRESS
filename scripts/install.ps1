...
# --- Pre-flight: Check for pnpm and node --------------------------------
...
if (-not (Get-Command pnpm -ErrorAction SilentlyContinue)) {
    Write-Host "    pnpm not found - attempting to install via npm..." -ForegroundColor Yellow
    if (Get-Command npm -ErrorAction SilentlyContinue) {
        npm install -g pnpm
        if ($LASTEXITCODE -ne 0) {
            Fail "Failed to install pnpm. Install it manually: npm install -g pnpm"
        }
    } else {
        Fail "Node.js/npm not found. Install Node.js first: https://nodejs.org/"
    }
}

Write-Progress-Done "Frontend tools available"

# --- Pre-flight: Install frontend dependencies if needed ----------------
...
if (Test-Path (Join-Path $frontendPath "package.json")) {
    ...
    try {
        ...
        Write-Progress-Done "Frontend dependencies ready"
    } finally {
        Pop-Location
    }
}
...
if ($FirstInstall) {
    ...
    Write-Progress-Done "Using existing .env"
}
...
Step "Discovering base images from Dockerfiles"
...
Step "Building app image (API + dashboard)"
...
Step "Building worker image (browser + detection engine)"
Write-Host "    This is the largest image, please be patient..." -ForegroundColor DarkGray
Build-InstallService "worker" "Building the worker image failed"

Step "Building beat scheduler image"
Write-Host "    Shares cache with worker, should be fast..." -ForegroundColor DarkGray
...
Write-Progress-Done "Dashboard is healthy and responding"
