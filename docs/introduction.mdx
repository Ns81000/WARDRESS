...
<img
  className="h-32 block dark:hidden"
...
```mermaid
graph TD
    UI[Frontend SPA<br>React/Vite] <--> API[FastAPI Web Server]
    API <--> DB[(PostgreSQL)]
    API <--> REDIS[(Redis Cache / Broker)]
    REDIS <--> CELERY[Celery Beat & Workers]
    CELERY --> PLAYWRIGHT[Playwright/Chromium]
