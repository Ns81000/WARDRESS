...
```mermaid
flowchart LR
    PR[Metadata prober · httpx] --> TLS[TLS certificate diff]
    PR --> HDR[Security header diff]
    PR --> RB[robots.txt diff]
    TLS --> SUM["score = min(1.0, tls + hdr + robots)"]
    HDR --> SUM
    RB --> SUM
    SUM --> OUT[Layer 6 score · 0.0–1.0]
