...
<Info>
  Source: `backend/worker/detection/dom.py` (`layer3_link_audit`, `_collect_refs`, `_norm_ref`, `_domains`). Input: the suppression-filtered, volatility-normalized HTML of both sides.
</Info>
...
flowchart LR
    HTML["Suppressed<br/>HTML"]:::in --> EX["Collect 5 ref kinds<br/>a · link · script · iframe · form"]:::node
    EX --> NM["Normalize · resolve<br/>drop js: data: mailto:"]:::node
    NM --> DM(["Hostname set"]):::node
    DM --> SD{"Set difference<br/>vs baseline"}:::pick
    SD -->|"added hosts"| ND["New external<br/>× weight"]:::node
    SD -->|"same host"| CH["Same-domain<br/>churn"]:::node
    ND & CH --> MX(("max")):::pick
    MX --> OUT["Layer 3<br/>0.0 – 1.0"]:::out

    classDef in fill:#004d40,stroke:#1de9b6,color:#fff;
    classDef node fill:#00695c,stroke:#4db6ac,color:#fff;
    classDef pick fill:#00bfa5,stroke:#a7ffeb,color:#00201b;
    classDef out fill:#00897b,stroke:#b2dfdb,color:#fff,stroke-width:3px;
```
...
<Steps>
  ...
  <Step title="Normalization">
    Each reference is normalized **browser-faithfully** before resolution: for `http`/`https` references, backslashes are treated as slashes (including any run of slashes/backslashes immediately after the scheme colon), exactly as browsers parse URLs — so a hijacked reference like `/\evil.com/x.js` attributes to `evil.com`, not to the page's own host. The result is then resolved against the page's `final_url` with `urljoin`. References that are not comparable links are dropped: fragments (`#...`), and `javascript:`, `data:`, `mailto:`, `tel:`, `about:` schemes. Only `http`, `https`, and scheme-relative refs survive. Fragments are stripped from the tail; **query strings are kept** (defacers love appending `?redirect=`). The one exception is the automatic volatile-text pass that ran before extraction: values of well-known cache-busting parameters (`v`, `ver`, `t`, `ts`, `cb`, `nocache`, … — value-shape-guarded, so `?v=3` survives) and UUID-shaped values in any query parameter are replaced with a stable placeholder on both sides, so a re-fetched asset no longer registers as a removed + added reference. URL paths and domains are never rewritten.
  </Step>
