...
```mermaid
flowchart LR
    HTML[Suppressed HTML] --> VT[Extract visible text]
    VT --> NT[New text vs baseline]
    NT --> AG[Aggression lexicon · weighted]
    NT --> TP[Topic keywords]
    VT --> EMB[MiniLM 384-d embeddings]
    EMB --> COS[Cosine · baseline vs current]
    AG --> M[max]
    TP --> M
    COS --> M
    M --> OUT[Layer 8 score · 0.0–1.0]
