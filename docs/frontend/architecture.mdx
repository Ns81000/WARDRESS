...
```mermaid
graph TD
    APP[App Entry: main.tsx] --> ROUTER[React Router]
    ROUTER --> AUTH{Auth Guard}
    AUTH -- Unauthenticated --> LOGIN[Login View]
    AUTH -- Authenticated --> LAYOUT[Main Dashboard Layout]
    
    LAYOUT --> DASH[Global Dashboard /sites]
    LAYOUT --> SCAN[Scan Detail /sites/:id/scans/:scanId]
    LAYOUT --> SETTINGS[Global Settings /settings]
    
    SCAN --> QUERY[React Query Cache]
